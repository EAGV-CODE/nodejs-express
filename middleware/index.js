...
middlewareObj.checkActivityzoneOwnership = function(req, res, next){
	if(req.isAuthenticated()){
		Activityzone.findById(req.params.id, function(err, foundActivityzone){
			if(err || !foundActivityzone){
				req.flash("error", "Activity Zone not found");
				res.redirect("back");
			}else{
				// does user own the activityzone?
				if(foundActivityzone.author.id.equals(req.user._id)){
					next();
				}else{
